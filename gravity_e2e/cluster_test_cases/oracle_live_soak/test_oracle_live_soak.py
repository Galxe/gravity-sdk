"""Long-running four-validator Oracle soak against Binance and Polygon."""
...
def _soak_settings() -> SoakSettings:
    ...
    if configured_minimum is None:
        minimum_advances = (
            0 if duration < 120 else max(1, (duration // 60) * 8 // 10)
        )
    else:
        minimum_advances = int(configured_minimum)
    ...
    if configured_restart is None:
        restart_after = duration // 2 if duration >= 60 * 60 else 0
    else:
        restart_after = int(configured_restart)
...
def _log_contains(
    cluster: Cluster, node_id: str, marker: str, issuer: str
) -> bool:
...
async def _wait_for_consensus_evidence(
    ...
    raise AssertionError(
        ...
        f"certifiers="
        f"{ {key: sorted(value) for key, value in certifiers.items()} } "
    ...
    return w3.eth.get_logs(
        {
            ...
            "topics": [
                ...
                support.topic(source_type),
                support.topic(source_id),
            ],
        }
    )
...
def _relayer_quorum(
    cluster: Cluster, uri: str, minimum_nonce: int
) -> list[str]:
    reached = []
    for node_id in cluster.nodes:
        raw_nonce = (
            _relayer_sources(cluster, node_id)
            .get(uri, {})
            .get("last_nonce")
        )
        try:
...
async def _replicated_snapshot(
    ...
    assert latest_price[1] == support.round_id(
        bucket_start_ms, price_progress[0]
    )
    assert latest_price[2] == support.round_end_ms(
        bucket_start_ms, price_progress[0]
    )
    assert price_progress[1] == latest_price[2]
    assert latest_price[3] == support.DECIMALS
...
async def _restart_validator(
    cluster: Cluster, node_id: str, target_block: int
) -> float:
...
async def _run_soak(
    ...
    while True:
        ...
        if (
            ...
            restart_recovery_seconds = await _restart_validator(
                cluster, settings.restart_node, int(final["block"])
            )
            restart_done = True
    ...
    final_bucket = support.round_start_ms(
    ...
    return {
        ...
        "restartNode": settings.restart_node
        if settings.restart_after_seconds
        else None,
...
async def test_governance_activated_oracles_soak_for_configured_duration(
    ...
    assert support.is_live_mode(), "oracle_live_soak must use live Binance"
    ...
    contracts_out = support.ensure_contracts_out(
        SUITE_DIR, required, "live Oracle soak"
    )
    ...
    receipt = await support.execute_governance_proposal(
        ...
        targets,
        datas,
        "activate-live-nvda-and-polymarket-soak",
        gas=8_000_000,
    )
    ...
    assert Web3.to_hex(settlement[5]).lower() == polymarket[
        "questionId"
    ].lower()
    ...
    first_bucket = support.round_start_ms(
    ...
    assert price_delivery_count == summary["finalNonce"], (
        "NVDA delivery event count does not match final source nonce"
    )
    ...
    for source_type, source_id, task_name in (
        (
            support.SOURCE_TYPE_PRICE_FEED,
            feed_id,
            support.TASK_PRICE_FEED,
    ...
    summary.update(
        {
            ...
            "priceDeliveryEvents": price_delivery_count,
            "polymarketDeliveryEvents": polymarket_delivery_count,
        }
    )

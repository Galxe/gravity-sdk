...
The current implementation keeps dynamic market discovery out of scope and focuses on the
settlement rail:
...
- Static task: configure known CTF condition ids in genesis or governance.
- Direct binary mirror: use `PolymarketBinaryMarket` for one reviewed
  two-outcome Polymarket condition such as a Fed-rate YES/NO market or a
  single-match "Team A wins?" market.
- Direct 3-way mirror: use `PolymarketMatchMarket` for one reviewed
  three-outcome CTF condition.

"""
Test basic ETH transfer functionality (Refactored)

This test verifies that ETH transfers work correctly using the new
utility modules for cleaner, more maintainable code.

Key improvements from refactoring:
- Uses TransactionBuilder for transaction creation and sending
- Uses centralized error handling with handle_test_exception
- Eliminates duplicate transaction building code
...
async def test_eth_transfer(run_helper: RunHelper, test_result: TestResult):
    """Test basic ETH transfer functionality using refactored utilities"""
    LOG.info("Starting ETH transfer test (refactored)")
    ...
    try:
        ...
        if not result.success:
            raise TransactionError(
                f"Transfer transaction failed: {result.error}",
        ...
        with handle_test_exception(test_result, reraise=False):
            raise e
        raise
...
async def test_multiple_transfers(run_helper: RunHelper, test_result: TestResult):
    ...
    LOG.info("Starting multiple transfers test (refactored)")
    ...
    try:
        ...
        for i, receiver in enumerate(receivers):
            ...
            if balance < transfer_amount:
                raise TransactionError(
                    f"Receiver {i} received insufficient funds"
                )
...
async def test_transfer_with_insufficient_funds(run_helper: RunHelper, test_result: TestResult):
    ...
    LOG.info("Starting insufficient funds test (refactored)")
    ...
    try:
        ...
        try:
            ...
            if sim_result['success']:
                ...
                if result.success:
                    raise TransactionError("Transaction with insufficient funds unexpectedly succeeded")
                else:

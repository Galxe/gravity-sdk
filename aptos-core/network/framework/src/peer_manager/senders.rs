...
use gaptos::aptos_channels::{self, gaptos::aptos_channel};
...
pub struct PeerManagerRequestSender {
    inner: gaptos::aptos_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>,
}
...
pub struct ConnectionRequestSender {
    inner: gaptos::aptos_channel::Sender<PeerId, ConnectionRequest>,
}

impl PeerManagerRequestSender {
    /// Construct a new PeerManagerRequestSender with a raw channel::Sender
    pub fn new(inner: gaptos::aptos_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>) -> Self {
        Self { inner }
    }
...
impl ConnectionRequestSender {
    /// Construct a new ConnectionRequestSender with a raw gaptos::aptos_channel::Sender
    pub fn new(inner: gaptos::aptos_channel::Sender<PeerId, ConnectionRequest>) -> Self {
        Self { inner }
    }

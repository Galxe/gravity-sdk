...
pub struct Peer<TSocket> {
    ...
    peer_reqs_rx: gaptos::aptos_channel::Receiver<ProtocolId, PeerRequest>,
    ...
    TSocket: AsyncRead + AsyncWrite + Send + 'static,
{
                ...
                match self.upstream_handlers.get(&direct.protocol_id) {
                    ...
                    Some(handler) => {
                        ...
                        match handler.push(key, ReceivedMessage::new(message, sender)) {
                            Err(_err) => {
                                // NOTE: gaptos::aptos_channel never returns other than Ok(()), but we might switch to tokio::sync::mpsc and then this would work
    ...
    async fn do_shutdown(
        mut self,
        write_req_tx: gaptos::aptos_channel::Sender<(), NetworkMessage>,

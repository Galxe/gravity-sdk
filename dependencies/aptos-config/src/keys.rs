...
impl<T: DeserializeOwned + PrivateKey + ValidCryptoMaterial + Serialize> ConfigKey<T> {
    ...
    pub fn public_key(&self) -> T::PublicKeyMaterial {
        gaptos::aptos_crypto::PrivateKey::public_key(&self.key)
    }
...
#[cfg(test)]
impl<T: PrivateKey + Serialize + gaptos::aptos_crypto::Uniform> Default for ConfigKey<T> {
    fn default() -> Self {
        Self {
            key: gaptos::aptos_crypto::Uniform::generate_for_testing(),
        }
    }
}
